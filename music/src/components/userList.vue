            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    ...
                    <el-button
                            size="mini"
                            type="warning"
                            @click="freeze(scope.row)" >封禁</el-button>
                </template>
            </el-table-column>
        </el-table>
        <!--修改会员信息 对话框-->
        <el-dialog title="会员信息" :visible.sync="dialogUpdateVisible" >
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button :plain="true" type="primary" @click="update('updateForm')">确 定</el-button>
            </div>
        </el-dialog>

        <!--添加会员信息 对话框-->
        <el-dialog title="会员信息" :visible.sync="dialogAddVisible" v-if="dialogAddVisible">
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button :plain="true" type="primary" @click="add('addForm')">确 定</el-button>
            </div>
        </el-dialog>
...
<script>
    export default {
        ...
        created(){
            ...
            /*this.$http.get("http://localhost:8082/muser/findByFlag?flag="+this.flag).then(req=>{
                this.tableData=req.data.list;
                //this.length=req.data.list.size();
                this.count=req.data.count;
            })*/
        },
        methods: {
                       ...
                       // alert(this.updateForm.u_id);
                        ...
                        this.$http.get("http://localhost:8082/muser/update?u_id="+this.updateForm.u_id+"&tel="+this.updateForm.tel+"&upwd="+this.updateForm.upwd+"&uname="+this.updateForm.uname+"&email="+this.updateForm.email+"&age="+this.updateForm.age+"&sex="+this.updateForm.sex+"&des="+this.updateForm.des+"&uphoto="+this.updateForm.uphoto).then(req=>{
                            if(req.data!=""){
                                ...
                                this.$message({
                                    showClose: true,
                                    message: '会员'+this.updateForm.u_id+'信息修改成功！',
                                    type: 'success'
                                });
            ...
            freeze:function(row){
               // alert(row.u_id);
                this.$http.get("http://localhost:8082/muser/freeze?u_id="+row.u_id).then(req=>{
                    if (req.data!=""){
                        /* alert(req.data);*/
                        this.$message({
                            showClose: true,
                            message: '会员'+row.u_id+'账号封禁成功！',
                            type: 'success'
                        });
                        this.reload();
                    }
